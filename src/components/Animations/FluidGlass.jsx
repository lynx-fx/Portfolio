...
export default function FluidGlass({ mode = 'lens', lensProps = {}, barProps = {}, cubeProps = {} }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', minHeight: '450px', position: 'relative', overflow: 'hidden' }}>
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  const fallbackGeo = useMemo(() => {
    if (fallbackShape === 'box') {
      const geo = new THREE.BoxGeometry(1, 1, 1);
      geo.computeBoundingBox();
      return geo;
    }
    if (fallbackShape === 'bar') {
      const geo = new THREE.BoxGeometry(3, 0.4, 0.2);
      geo.computeBoundingBox();
      return geo;
    }
    const geo = new THREE.CylinderGeometry(1, 1, 0.35, 32);
    geo.computeBoundingBox();
    return geo;
  }, [fallbackShape]);
  ...
  useFrame((state, delta) => {
    ...
    // Transparent / sleek dark theme background clear
...
function Lens({ modeProps, ...p }) {
  return <ModeWrapper fallbackShape="cylinder" followPointer modeProps={modeProps} {...p} />;
}

function Cube({ modeProps, ...p }) {
  return <ModeWrapper fallbackShape="box" followPointer modeProps={modeProps} {...p} />;
}
...
function Images() {
  ...
  return (
    <group ref={group}>
      <Image position={[-2, 0, 0]} scale={[3, height / 1.1, 1]} url="/Voyage.png" />
      <Image position={[2, 0, 3]} scale={3} url="/CardKeeper.png" />
      <Image position={[-2.05, -height, 6]} scale={[1, 3, 1]} url="/Serviq.png" />
      <Image position={[-0.6, -height, 9]} scale={[1, 2, 1]} url="/Eventra.png" />
      <Image position={[0.75, -height, 10.5]} scale={1.5} url="/Vanis.png" />
    </group>
  );
}
